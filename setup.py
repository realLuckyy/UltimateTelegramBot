...
def update_bot_file(token):
    ...
    # Replace the token
    old_pattern = r'TOKEN\s*=\s*os\.getenv\([^)]*,\s*["\'][^"\']*["\'][^)]*\)'
    new_token_line = f'TOKEN = os.getenv(\'BOT_TOKEN\', "{token}")'
    
    if re.search(old_pattern, content):
        content = re.sub(old_pattern, new_token_line, content)
    else:
        # Fallback to simpler pattern
        old_simple_pattern = r'TOKEN\s*=\s*["\'][^"\']*["\']'
        new_simple_line = f'TOKEN = "{token}"'
        if re.search(old_simple_pattern, content):
            content = re.sub(old_simple_pattern, new_simple_line, content)
        else:
...
def show_next_steps():
    ...
    print()
    print("Next steps:")
    print("1??  Run the bot: python UltimateTelegrambot.py")
    print("2??  Add bot to your Telegram group")
    print("3??  Make bot an administrator with these permissions:")
    print("    • Delete Messages")
    print("    • Ban Users")  
    print("    • Invite Users")
    print("4??  Send /help in the group to test")
    print()
    print("?? Documentation:")
    ...
    print("? Support development: https://ko-fi.com/root")
    print()
